...
public class GamemodeSpectatorCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if(!(p.hasPermission("earth.spectator"))){
            p.sendMessage("§aEARTH §8| §cInsufficient permissions");
            return false;
        }
        if(args.length == 0){
            p.setGameMode(GameMode.SPECTATOR);
            p.sendMessage("§aEARTH §8| §7You changed your gamemode to §aSpectator§7!");
            return false;
        }
        if(args.length == 1){
            ...
            p.sendMessage("§aEARTH §8| §7You changed §6" + player.getName() + " §7gamemode to §aSpectator§7!");
            player.sendMessage("§aEARTH §8| §6" + p.getName() + " §7set your gamemode to §aSpectator§7!");
        }
        return false;
    }
}
